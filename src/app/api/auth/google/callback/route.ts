...
if (
    ...
    console.error("Missing required environment variables for Google Callback.");
    throw new Error(
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (!email || !emailVerified) {
            ...
            return NextResponse.redirect(`${appUrl}/waitlist/denied?reason=email_unverified`);
        }

        if (!email.endsWith("@kkwagh.edu.in")) {
            console.warn(
                `User denied: Email domain mismatch`
            );
        ...
        const { data: existingUser, error: selectError } = await supabaseAdmin.from("waitlist_users").select("hashed_google_id").eq("hashed_google_id", hashedGoogleId).maybeSingle();
        ...
        const { error: insertError } = await supabaseAdmin.from("waitlist_users").insert({ hashed_google_id: hashedGoogleId });
        ...
        console.log(`Successfully added user ${hashedGoogleId} to the waitlist.`);
        cookieStore.set({
            name: 'waitlisted',
            value: 'yes',
            httpOnly: true,
